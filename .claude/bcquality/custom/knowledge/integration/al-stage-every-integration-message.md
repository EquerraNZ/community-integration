...
The Integration Message is a normal table whose rows carry everything a processor needs to act without calling back to the source: the document number, the message type (an extensible enum with interface-driven dispatch), a status (New, In Progress, Completed, Failed), the request and response content blobs, an idempotency key for dedup, error content and error code for classification, a correlation id for tracing, and a parent message id for staged pipelines.
...
Expose the Integration Message as a single API page and let the Type enum route each row to the correct handler via its interface implementation, rather than versioning a separate endpoint per source system. One endpoint plus a Type-driven dispatcher means a new message kind is a new enum value implementing the handler interface, not a new published API surface. Keep the Idempotency Key unique so a replayed delivery is detected at insert time rather than processed twice. See `al-stage-every-integration-message.good.al` for the intake page, the staging table shape, and the Job Queue processor.
